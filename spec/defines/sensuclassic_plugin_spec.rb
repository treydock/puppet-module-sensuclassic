...
describe 'sensuclassic::plugin', :type => :define do
  ...
  context 'file' do
    ...
    context 'running on Linux' do
      context 'defaults' do
        it do
          should contain_sensuclassic__plugins_dir('puppet:///data/plug1-/etc/sensu/plugins').with({
    ...
    context 'running on Windows' do
      ...
      context 'defaults' do
        it do
          should contain_sensuclassic__plugins_dir('puppet:///data/plug1-C:/opt/sensu/plugins').with({
  ...
  context 'url' do
    ...
    context 'running on Linux' do
      context 'defaults' do
        it do
          should contain_sensuclassic__plugins_dir('https://raw.githubusercontent.com/sensu/sensu-community-plugins/master/plugins/system/check-mem.sh-/etc/sensu/plugins').with({
            :path   => '/etc/sensu/plugins',
          })
        end
    ...
    context 'running on Windows' do
      ...
      context 'defaults' do
        it do
          should contain_sensuclassic__plugins_dir('https://raw.githubusercontent.com/sensu/sensu-community-plugins/master/plugins/system/check-mem.sh-C:/opt/sensu/plugins').with({
            :path   => 'C:/opt/sensu/plugins',
          })
        end
  ...
  context 'package' do
    ...
    context 'set pkg_provider' do
      let(:params) { { :type => 'package', :pkg_provider => 'sensuclassic_gem' } }

      it { should contain_package('sensu-plugins').with_provider('sensuclassic_gem') }
    end
  ...
  let(:pre_condition) do
    ...
    describe 'notifies the sensu-client service' do
      ...
      it { is_expected.to contain_sensuclassic__plugin(title).with(expected)}
    end
