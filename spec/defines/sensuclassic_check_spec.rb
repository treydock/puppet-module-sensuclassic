...
describe 'sensuclassic::check', :type => :define do
  ...
  context 'with content' do
    ...
    context 'containing sensu-plugins-mailer configuration' do
      ...
      it { should contain_sensuclassic__write_json(fpath).with(
  ...
  context 'without whitespace in name' do
    context 'defaults' do
      ...
      it do
        ...
        should contain_sensuclassic__write_json(fpath).with(rsrc_hsh)
      end
    end
    ...
    context 'ensure absent' do
      ...
      it { should contain_sensuclassic__write_json(fpath).with_ensure('absent') }
    end
  ...
  context 'with whitespace in name' do
    ...
    context 'defaults' do
      it { should contain_sensuclassic__write_json(fpath) }
  ...
  context 'notifications' do
    context 'no client, sever, or api' do
      let(:pre_condition) { 'class {"sensuclassic": client => false, api => false, server => false}' }
      it { should contain_sensuclassic__write_json(fpath).with(:notify_list => []) }
    end

    context 'only client' do
      let(:pre_condition) { 'class {"sensuclassic": client => true, api => false, server => false}' }
      it { should contain_sensuclassic__write_json(fpath).with(:notify_list => ['Service[sensu-client]'] ) }
    end

    context 'only server' do
      let(:pre_condition) { 'class {"sensuclassic": client => false, api => false, server => true}' }
      it { should contain_sensuclassic__write_json(fpath).with(:notify_list => ['Class[Sensuclassic::Server::Service]'] ) }
    end

    context 'only api' do
      let(:pre_condition) { 'class {"sensuclassic": client => false, api => true, server => false}' }
      it { should contain_sensuclassic__write_json(fpath).with(:notify_list => ['Service[sensu-api]'] ) }
    end

    context 'client and api' do
      let(:pre_condition) { 'class {"sensuclassic": client => true, api => true, server => false}' }
      it { should contain_sensuclassic__write_json(fpath).with(:notify_list => ['Service[sensu-client]', 'Service[sensu-api]']) }
    end

    context 'client and server' do
      let(:pre_condition) { 'class {"sensuclassic": client => true, api => false, server => true}' }
      it { should contain_sensuclassic__write_json(fpath).with(:notify_list => ['Service[sensu-client]', 'Class[Sensuclassic::Server::Service]']) }
    end

    context 'api and server' do
      let(:pre_condition) { 'class {"sensuclassic": client => false, api => true, server => true}' }
      it { should contain_sensuclassic__write_json(fpath).with(:notify_list => ['Class[Sensuclassic::Server::Service]', 'Service[sensu-api]']) }
    end
  ...
  describe 'relationships (#463)' do
    let(:expected) { { notify: ["Sensuclassic::Check[#{title}]"] } }
  ...
  describe 'params conversions where arrays are expected (#803)' do
    context 'handlers is passed as String' do
      ...
      it { should contain_sensuclassic__write_json(fpath).with_content("checks"=>{"mycheck"=>{"standalone"=>true, "command"=>"/etc/sensu/somecommand.rb", "handlers"=>["default"], "interval"=>60}}) }
    end
    context 'subscribers is passed as String' do
      let(:params_override) { {subscribers: 'default'} }
      it { should contain_sensuclassic__write_json(fpath).with_content("checks"=>{"mycheck"=>{"standalone"=>true, "command"=>"/etc/sensu/somecommand.rb", "subscribers"=>["default"], "interval"=>60}}) }
    end
    context 'aggregates is passed as String' do
      let(:params_override) { {aggregates: 'default'} }
      it { should contain_sensuclassic__write_json(fpath).with_content("checks"=>{"mycheck"=>{"standalone"=>true, "command"=>"/etc/sensu/somecommand.rb", "aggregates"=>["default"], "interval"=>60}}) }
    end
    context 'dependencies is passed as String' do
      let(:params_override) { {dependencies: 'default'} }
      it { should contain_sensuclassic__write_json(fpath).with_content("checks"=>{"mycheck"=>{"standalone"=>true, "command"=>"/etc/sensu/somecommand.rb", "dependencies"=>["default"], "interval"=>60}}) }
    end
    context 'contacts is passed as String' do
      let(:params_override) { {contacts: 'default'} }
      it { should contain_sensuclassic__write_json(fpath).with_content("checks"=>{"mycheck"=>{"standalone"=>true, "command"=>"/etc/sensu/somecommand.rb", "contacts"=>["default"], "interval"=>60}}) }
    end
    ...
    context 'handlers is passed as absent' do
      ...
      it { should contain_sensuclassic__write_json(fpath).with_content("checks"=>{"mycheck"=>{"standalone"=>true, "command"=>"/etc/sensu/somecommand.rb", "interval"=>60}}) }
    end
